...
<div class="Consigneew boxS w1190">
	<div class="Consignee clearfix">
		<form id="recForm" action="${pageContext.request.contextPath }/order/save.do" method="post">
			<input type="hidden" id="recId" name="recId" value="">
			<input type="hidden" id="recName" name="recName" value="">
			<input type="hidden" id="recAdd" name="recAdd" value="">
			<input type="hidden" id="recPhone" name="recPhone" value="">
		</form>
		<h3 class="Contitle">
			收货人信息
		</h3>
		<ul class="address clearfix">
			<c:forEach items="${ requestScope.recList }" var="rec">
				<c:choose>
					<c:when test="${rec.recChecked eq 1 }">
						<li class="checkbox checked"
							recId="${rec.recId }" recName="${rec.recName }"
							recAdd="${rec.recAdd }" recPhone="${rec.recPhone}">
							<p class="user">${rec.recName }</p>
							<p class="site">${rec.recAdd }</p>
							<p class="phonenum">${rec.recPhone}</p>
						</li>
					</c:when>
					<c:otherwise>
						<li class="checkbox" recId="${rec.recId }" recName="${rec.recName }"
							recAdd="${rec.recAdd }" recPhone="${rec.recPhone}">
							<p class="user">${rec.recName }</p>
							<p class="site">${rec.recAdd }</p>
							<p class="phonenum">${rec.recPhone}</p>
	...
	<div class="Blur">
		...
		<div class="BlurArea">
			...
			<div class="prouduct clearfix">
				<c:set var="priceCount" scope="page" value="0"/>
				<c:forEach items="${requestScope.itemList }" var="item">
					<ul class="blur clearfix">
						<li class="first">
							<p>${item.itemGoods.goodsName }</p>
						</li>
						<li>${item.itemGoodsNum }</li>
						<li>￥${item.itemGoods.goodsPrice }</li>
						<li>200g</li>
						<li>￥0</li>
						<li>￥${item.itemGoods.goodsPrice*item.itemGoodsNum}</li>
					</ul>
					<c:set var="priceCount" scope="page"
						   value="${pageScope.priceCount + (item.itemGoods.goodsPrice*item.itemGoodsNum)  }"/>
				</c:forEach>
			</div>
			...
			<span class="right">
				<em>合计</em>
				<em class="r">￥${pageScope.priceCount}</em>
			</span>
			</div>
		</div>
		<div class="blurFooterw clearfix">
			<div class="blurFooter">
				...
				<p class="submit clearfix">
					<span>返回购物车</span>
					<span>实付总额： <em>￥${pageScope.priceCount}</em></span>
					<a href="javascript:void(0)" id="tosaveorder">提交订单</a>
				</p>
...
<script>
	// 提交订单
	$(function() {
		$(".address .checkbox[class='checkbox checked']").click();
