...
<div class="OrderInfow w1190 boxS">
	<div class="OrderInfo">
		...
		<div class="OrderInAreadown">
			<p class="left">
			<c:forEach items="${requestScope.order.detailList }" var="detail">
				<span>
					${detail.detailGoods.goodsName }&nbsp;|&nbsp;${detail.detailGoodsPrice }&nbsp;X&nbsp;${detail.detailGoodsNum }&nbsp;=&nbsp;${detail.detailGoodsNum * detail.detailGoodsPrice}
				</span>
			</c:forEach>
				 <span class="c99">
				 	收货信息 : ${requestScope.order.orderAddress }&nbsp;${requestScope.order.orderConsignee }
				 	&nbsp;${requestScope.order.orderPhone }
				 </span>
			</p>
		</div>
	</div>
...
<div class="wayw w1190 boxS">
	<div class="way">
		<h3>
			支付金额 ：<b>￥<fmt:formatNumber value="${requestScope.order.orderPrice }" pattern=".00" /></b>
			<span>
				关于实名验证?
				<u>
					<b></b>
		...
		<form id="payForm" action="${pageContext.request.contextPath}/order/pay.do" method="post">
<%--			订单id--%>
			<input type="hidden" name="orderId" value="${requestScope.order.orderId}"/>
			<input type="hidden" name="orderPrice" value="${requestScope.order.orderPrice}"/>
			<input type="hidden" name="orderName" value="${requestScope.order.orderAddress}|${requestScope.order.orderConsignee}|${requestScope.order.orderPhone}">
		</form>
		<div class="wayArea">
			<ul class="clearfix">
				<li class="clearfix">
			...
			<a href="javascript:void(0)" id="paySubmit_a">用支付宝支付</a>
		</div>
	</div>
</div>
<!-- 底部 -->
